...
function NavLink({ children, href, target = "_self" }: NavLinkProps) {
	return (
		<Link
			...
			target={target}
			className={
				"h-full w-auto ml-4 flex items-center transition-all hover:text-accent"
			}>
			{children}
		</Link>
	);
...
export default function Navbar() {
	return (
		<Container>
			<div
				className={
					"h-20 w-full relative flex justify-center md:justify-between items-center py-5 top-0"
				}
				id={"nav"}>
				<Link
					href={"/"}
				...
				<div className={"h-full w-max justify-end hidden md:flex"}>
					...
					<NavLink href={"/contact-me"}>
						<span className={"text-accent mr-2"}>03.</span>Contact
						Me
					</NavLink>
				</div>
			</div>
